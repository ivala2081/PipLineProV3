...
from app.utils.api_response import success_response, error_response, ErrorCode, paginated_response
from app.utils.api_error_handler import handle_api_errors
from app.utils.constants import MAX_REVENUE_TRENDS_DAYS, MAX_DASHBOARD_ITEMS
...
csrf.exempt(analytics_api)  # Still exempt blueprint, but use @require_csrf on critical routes
...
def dashboard_stats():
    ...
    dashboard_data = {
        ...
        'chart_data': {
            # Limit revenue trends for performance (even for 'all' time range)
            'daily_revenue': chart_data.get('daily_revenue', [])[-MAX_REVENUE_TRENDS_DAYS:]
        },
        'revenue_trends': chart_data.get('daily_revenue', [])[-MAX_REVENUE_TRENDS_DAYS:]  # Limited for performance
    }
...
def test_csrf():
    ...
    try:
        ...
        if current_app.config.get('DEBUG', False):
            logger.debug(f"CSRF tokens - Header: {bool(request.headers.get('X-CSRFToken'))}, Session: {bool(session.get('csrf_token'))}")
