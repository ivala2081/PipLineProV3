...
from app.utils.unified_error_handler import DatabaseError
...
def get_transactions():
    ...
    MAX_PER_PAGE = 500  # Maximum allowed per_page (reduced from 10000 for performance)
    DEFAULT_PER_PAGE = 25
    requested_per_page = request.args.get('per_page', DEFAULT_PER_PAGE, type=int)
    per_page = min(max(1, requested_per_page), MAX_PER_PAGE)  # Ensure between 1 and MAX_PER_PAGE
    
    # Log warning if client requested more than max (helps identify code that needs updating)
    if requested_per_page > MAX_PER_PAGE:
        logger.warning(
            f"Client requested per_page={requested_per_page} but max is {MAX_PER_PAGE}. "
