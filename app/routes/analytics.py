...
def analytics():
    ...
    try:
        ...
        return serve_frontend('/analytics')
...
def business_analytics():
    ...
    try:
        ...
        return serve_frontend('/business-analytics')
...
def psp_track():
    ...
    try:
        ...
        return serve_frontend('/psp-track')
    ...
    if request.method == 'POST':
        try:
            ...
            if not name:
                ...
                from app.utils.frontend_helper import serve_frontend
                return serve_frontend('/employees/add')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if not name:
                flash('Employee name is required.', 'error')
                from app.utils.frontend_helper import serve_frontend
                return serve_frontend(f'/employees/edit/{employee_id}')
...
def agent_management():
    ...
    try:
        ...
        return serve_frontend('/agent-management')
...
def view_psp_track(psp, date):
    ...
    try:
        ...
        return serve_frontend('/psp-track/view')
...
def add_psp_track():
    ...
    try:
        ...
        return serve_frontend('/psp-track/add')
...
@login_required
def reports():
    """Reports page - serve React frontend"""
    try:
        from flask import send_from_directory
        frontend_dist = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'frontend', 'dist')
        if not os.path.exists(frontend_dist):
