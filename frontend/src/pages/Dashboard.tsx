...
const Dashboard = memo(() => {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          if (data.data?.psps?.length > 0) {
            console.log('✅ PSP Rollover Data Loaded:', `Loaded ${data.data.psps.length} PSPs with rollover data`);
          } else {
            console.log('ℹ️ No PSP Data:', 'No PSP transactions found in the database');
          }
        } else {
  ...
  const handleFetchDashboardData = useCallback(async (forceRefresh = false) => {
    try {
      ...
      if (!forceRefresh && (now - lastFetchTime) < CACHE_DURATION) {
        console.log('📊 Dashboard data is cached, skipping fetch');
        return;
      }

      console.log('📊 Fetching dashboard data...', { forceRefresh, timeRange });
  ...
  const handleRefresh = useCallback(async () => {
    try {
      console.log('🔄 Refreshing Dashboard:', 'Loading latest data...');
      dispatch(setRefreshing(true));
      await handleFetchDashboardData(true);
      console.log('✅ Dashboard Refreshed:', 'All data has been updated successfully');
    } catch (error) {
  ...
  const handleExchangeRatesRefresh = useCallback(async () => {
    try {
      console.log('🔄 Refreshing Exchange Rates:', 'Updating currency rates...');
      ...
      if (success) {
        console.log('✅ Exchange Rates Updated:', 'Currency rates have been successfully refreshed');
      } else {
  ...
  useEffect(() => {
    if (isAuthenticated && !authLoading) {
      console.log('🔄 Time range changed, fetching new data for:', timeRange);
