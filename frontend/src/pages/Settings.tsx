...
const SecurityManagement = lazy(() => import('../components/SecurityManagement'));
...
export default function Settings() {
  ...
  const navigateToDatabaseManagement = () => {
    ...
    const url = new URL(window.location.href);
    url.searchParams.set('tab', 'admin');
    url.searchParams.set('view', 'database');
    window.history.pushState({}, '', url.toString());
  };
  ...
  const navigateToSecurity = () => {
    ...
    const url = new URL(window.location.href);
    url.searchParams.set('tab', 'admin');
    url.searchParams.set('view', 'security');
    window.history.pushState({}, '', url.toString());
  };
  ...
  const handleAddOption = async () => {
    ...
    try {
      ...
      if (response.ok || response.status === 201) {
        ...
        console.log('✅ Success:', 'Option added successfully!');
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          console.log('✅ Option updated:', responseData);
        } catch (parseError) {
        ...
        try {
          setLoadingOptions(true);
          const refreshResponse = await api.get('/transactions/dropdown-options', undefined, false);
          if (refreshResponse.ok) {
            const data = await api.parseResponse(refreshResponse);
            console.log('📊 Refreshed dropdown data:', data);
            console.log('📊 PSP options:', data?.psp);
            setDropdownOptions(data || {});
        ...
        console.log('✅ Success:', 'Option updated successfully!');
      } else {
  ...
  const handleDeleteOption = async (option: DropdownOption, fieldName: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // İşlem başarılı olduktan sonra token'i temizle
        api.clearToken();
        console.log('✅ Success:', 'Option deleted successfully!');
      } else {
